...
import { OK_W4_FormData } from '../../types';
...
interface Props {
  data: OK_W4_FormData;
  onChange?: (data: OK_W4_FormData) => void;
  onSave?: (data: OK_W4_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof OK_W4_FormData, string>>;
}

const OKW4Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<OK_W4_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: OK_W4_FormData) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    updateState({ ...formData, [name]: value as OK_W4_FormData['filingStatus'] });
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">Oklahoma Form OK-W-4</h2>
