...
import { ME_W4_FormData, MEFilingStatus } from '../../types';
...
interface Props {
  data: ME_W4_FormData;
  onChange?: (data: ME_W4_FormData) => void;
  onSave?: (data: ME_W4_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof ME_W4_FormData, string>>;
}

const MEW4Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<ME_W4_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: ME_W4_FormData) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    updateState({ ...formData, [name]: value as MEFilingStatus });
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">Maine Form W-4ME</h2>
          ...
          <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              <div className="sm:col-span-3">
                  <Select id="filingStatus" name="filingStatus" label="Filing Status" value={formData.filingStatus} onChange={handleChange} error={errors?.filingStatus}>
                    {Object.values(MEFilingStatus).map(status => <option key={status} value={status}>{status}</option>)}
                  </Select>
              </div>
              <div className="sm:col-span-3">
