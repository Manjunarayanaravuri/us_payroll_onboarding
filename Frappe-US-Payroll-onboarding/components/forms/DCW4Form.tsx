...
import { DC_W4_FormData } from '../../types';
...
interface Props {
  data: DC_W4_FormData;
  onChange?: (data: DC_W4_FormData) => void;
  onSave?: (data: DC_W4_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof DC_W4_FormData, string>>;
}

const DCW4Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<DC_W4_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: DC_W4_FormData) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">District of Columbia Form D-4</h2>
        ...
        <div className="pt-8">
          ...
          <div className="mt-6 space-y-4">
              ...
              {formData.isExempt && (
                <Input id="exemptionReason" name="exemptionReason" label="Reason for Exemption" value={formData.exemptionReason || ''} onChange={handleChange} placeholder="e.g., Expect no DC tax liability"/>
              )}
          </div>
        </div>
