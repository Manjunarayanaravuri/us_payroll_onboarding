...
import { WV_IT104_FormData } from '../../types';
...
interface Props {
  data: WV_IT104_FormData;
  onChange?: (data: WV_IT104_FormData) => void;
  onSave?: (data: WV_IT104_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof WV_IT104_FormData, string>>;
}

const WVIT104Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<WV_IT104_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: WV_IT104_FormData) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">West Virginia Form WV/IT-104</h2>
        ...
        <div className="pt-8">
          ...
          <div className="mt-6 space-y-4">
              ...
              {formData.isExempt && (
                <Input id="exemptionReason" name="exemptionReason" label="Reason for Exemption" value={formData.exemptionReason || ''} onChange={handleChange} placeholder="e.g., Expect no WV tax liability"/>
              )}
          </div>
        </div>
      </div>
