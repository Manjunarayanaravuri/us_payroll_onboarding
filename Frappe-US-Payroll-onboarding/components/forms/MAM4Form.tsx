...
import { MA_M4_FormData } from '../../types';
...
interface Props {
  data: MA_M4_FormData;
  onChange?: (data: MA_M4_FormData) => void;
  onSave?: (data: MA_M4_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof MA_M4_FormData, string>>;
}

const MAM4Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<MA_M4_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: MA_M4_FormData) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">Massachusetts Form M-4</h2>
          ...
          <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
           ...
           <div className="mt-6 space-y-4">
              <Checkbox id="isExempt" name="isExempt" label="I claim exemption from withholding." checked={formData.isExempt} onChange={handleChange} />
               {formData.isExempt && (
                <Input id="exemptionReason" name="exemptionReason" label="Reason for Exemption" value={formData.exemptionReason || ''} onChange={handleChange} placeholder="e.g., Expect no MA tax liability"/>
              )}
          </div>
        </div>
