...
import { NC_W4_FormData } from '../../types';
...
interface Props {
  data: NC_W4_FormData;
  onChange?: (data: NC_W4_FormData) => void;
  onSave?: (data: NC_W4_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof NC_W4_FormData, string>>;
}

const NCW4Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<NC_W4_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: NC_W4_FormData) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    updateState({ ...formData, [name]: value as NC_W4_FormData['filingStatus'] });
  };
  ...
  const filingStatuses = {
    single: 'Single',
    married_jointly: 'Married or Qualifying Widow(er)',
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">North Carolina Form NC-4</h2>
