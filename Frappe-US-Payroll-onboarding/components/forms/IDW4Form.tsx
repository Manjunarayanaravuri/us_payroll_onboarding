...
import { ID_W4_FormData } from '../../types';
...
interface Props {
  data: ID_W4_FormData;
  onChange?: (data: ID_W4_FormData) => void;
  onSave?: (data: ID_W4_FormData) => void;
  isSaving?: boolean;
  errors?: Partial<Record<keyof ID_W4_FormData, string>>;
}

const IDW4Form: React.FC<Props> = ({ data, onChange, onSave, isSaving, errors }) => {
  const [formData, setFormData] = useState<ID_W4_FormData>(data);

  useEffect(() => { setFormData(data); }, [data]);

  const updateState = (updatedData: ID_W4_FormData) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900">Idaho Form ID W-4</h2>
        ...
        <div className="pt-8">
          ...
          <div className="mt-6 sm:col-span-3">
              <Input id="allowances" name="allowances" type="number" label="Total Idaho Allowances" value={formData.allowances} onChange={handleNumberChange} error={errors?.allowances} />
          </div>
        </div>
        ...
        <div className="pt-8">
          ...
          <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
           ...
           <div className="mt-6 space-y-4">
              ...
              {formData.isExempt && (
                <Input id="exemptionReason" name="exemptionReason" label="Reason for Exemption" value={formData.exemptionReason || ''} onChange={handleChange} placeholder="e.g., Expect no ID tax liability"/>
              )}
          </div>
        </div>
